...
![LensClaw Logo](docs/cyber_logo.png)

# LensClaw

###  **当 AI 开始"黑箱操作"，你需要一双透视眼**

[![LensClaw](https://img.shields.io/badge/LensClaw-1.0.0-purple.svg?logo=cyberpunk)](https://github.com/ttguy0707/LensClaw)
...
> 🤖 **你的 AI 在背着你做什么？LensClaw 让所有行为无所遁形**
> 
> 💡 **灵感来源**：受 [OpenClaw](https://github.com/openclaw/openclaw) 的启发，LensClaw 专注于解决 AI 智能体的透明度和可控性问题。

---

## 📖 简介

LensClaw 是一个**企业级透明可控智能体**，重新定义 AI 系统的可信边界：
...
LensClaw 支持**OpenClaw 技能**和**Claude Code 技能**，可直接使用两个生态系统的丰富技能资源，无需重新开发。
...
- **推荐技能**：
  - `skill-creator`：用自然语言让 LensClaw 自己创建技能
...
```bash
# 克隆项目
git clone https://github.com/ttguy0707/LensClaw.git
cd LensClaw
...
> 💡 **工作区配置**：工作区路径已在代码中初始化，默认为项目根目录的 `workspace` 文件夹，无需在 `.env` 中配置。仅当需要自定义工作区位置时，才设置 `LENSCLAW_WORKSPACE` 环境变量。

> 💡 提示：配置完成后，可运行 `lensclaw run` 聊天测试连接是否正常。
...
| 模块 | 文件 | 功能 |
|------|------|------|
| **Agent 循环** | `lensclaw/core/agent.py` | LangGraph StateGraph，决策大脑 |
| **技能加载** | `lensclaw/core/skill_loader.py` | 动态加载 SKILL.md，两段式调用 |
| **上下文管理** | `lensclaw/core/context.py` | 消息修剪，双水位记忆 |
| **内置工具** | `lensclaw/core/tools/builtins.py` | 时间/计算/任务调度等 |
| **沙盒工具** | `lensclaw/core/tools/sandbox_tools.py` | 文件操作 + Shell 执行 |
| **审计日志** | `lensclaw/core/logger.py` | JSONL 格式事件记录 |
| **心跳任务** | `lensclaw/core/heartbeat.py` | 定时任务检查与触发 |
...
[![Star History Chart](https://api.star-history.com/svg?repos=ttguy0707/LensClaw&type=Date)](https://star-history.com/#ttguy0707/LensClaw&Date)
