...
from lensclaw.core.agent import create_agent_app
from lensclaw.core.config import DB_PATH
from lensclaw.core.bus import task_queue
from lensclaw.core.heartbeat import pacemaker_loop
...
{RESET}"""
    ...
    sub_title = f"{WHITE}{BOLD} 👾 Welcome to the {PURPLE}{BOLD}LensClaw{RESET}{WHITE}{BOLD} !  {RESET}"
    ...
    tip = (
        f"{PURPLE} ✦ {RESET}"
        f"{SILVER}{PURPLE}{BOLD}LensClaw{RESET} 已完成启动。输入命令开始，输入 {PURPLE}/exit{RESET}{SILVER} 退出。{RESET}\n"
    )
        ...
        async def user_input_loop():
            ...
            while True:
                try:
                    ...
                    if user_input.lower() in ["/exit", "/quit"]:
                        cprint("  \033[38;5;141m✦ 记忆已固化，LensClaw 进入休眠。\033[0m")
                        break
                        
                except (KeyboardInterrupt, EOFError):
                    cprint("\n  \033[38;5;141m✦ 强制中断，LensClaw 进入休眠。\033[0m")
                    await task_queue.put("/exit")
                    break

            redraw_task.cancel() 

        if os.environ.get("LENSCLAW_DEBUG") == "1":
